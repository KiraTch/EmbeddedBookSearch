...
def load_books_data():
    ...
    try:
        ...
        books_df['Date'] = books_df['Date'].fillna('No release date available')
        ...
        books_df['Pages'] = pd.to_numeric(books_df['Pages'], errors='coerce').fillna(300)
...
def generate_or_load_embeddings(books_df, max_books=1000):
    ...
    if len(books_df) > max_books:
        cute_print(
            f"Dataset has {len(books_df)} books. Using top {max_books} highest-rated books for faster processing...",
            "⚡")
...
def interactive_chat():
    ...
    cute_print("Welcome to Your AI-Powered Book Recommender! 🤖📚", "🎉")
